...
class TensorSearchHelper(BasePage):
    ...
    def check_images(self, locator):
        ...
        flag = all([img.get_attribute("width") == images[0].get_attribute("width") for img in images]) and \
                all([img.get_attribute("height") == images[0].get_attribute("height") for img in images])
        if flag:
            self.logger.info("All images have equal dimensions")
        else:
            self.logger.error("Images have different dimensions or images not found")
        return flag
